...
import { effectFields } from "../domain/rule";
...
export function RuleForm({data,rule,remove=false}:{data:Data;rule?:Data["rules"][number];remove?:boolean}){
 const runtime=data.canEditEffects===true;
 ...
 return <form action={action} className="mt-4 space-y-3" onChange={()=>{if(!busy.current&&!rule)id.current=null;}} onSubmit={e=>{
  ...
  {runtime&&<input type="hidden" name="runtime" value="true"/>}
  ...
  <fieldset disabled={pending} className="space-y-3">
   {remove?<label className="flex gap-2"><input type="checkbox" name="confirmed" required/>Confirmo eliminar esta regla.</label>:<>
    ...
    {runtime&&<label className="block">Motivo del cambio<textarea name="reason" required maxLength={2000} className={field}/></label>}
    <h4 className="font-bold">Efectos</h4>
    {effects.map(e=>{const value=effectFields(e.amount);return <div key={e.key} className="grid gap-3 rounded-xl border border-white/10 p-3 sm:grid-cols-3">
     <label>KPI{runtime?<><input type="hidden" name="kpiId" value={e.kpiId}/><p>{data.kpis.find(k=>k.id===e.kpiId)?.name}</p></>:<select name="kpiId" required defaultValue={e.kpiId} className={field}><option value="">Seleccionar indicador</option>{data.kpis.map(k=><option key={k.id} value={k.id}>{k.name} ({k.unit})</option>)}</select>}</label>
     <label>Operación<select name="direction" defaultValue={value.direction} className={field}><option value="increase">Aumentar</option><option value="decrease">Disminuir</option></select></label>
     <label>Valor<input name="magnitude" required maxLength={64} inputMode="decimal" pattern="[0-9]+([.][0-9]+)?" defaultValue={value.magnitude} placeholder="Ej.: 5000" className={field}/></label>
     {!runtime&&<button type="button" className="underline" onClick={()=>{if(!rule)id.current=null;setEffects(old=>old.filter(v=>v.key!==e.key));}}>Quitar efecto</button>}
    </div>;})}
    {!runtime&&<button type="button" disabled={!data.kpis.length||effects.length>=data.kpis.length} className="underline disabled:opacity-50" onClick={()=>{if(!rule)id.current=null;const key=nextRow.current++;setEffects(old=>[...old,{key,kpiId:"",amount:""}]);}}>+ Agregar efecto</button>}
