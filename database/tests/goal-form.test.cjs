...
function render(pending=false) {
 ...
 const code=ts.transpileModule(fs.readFileSync(base+'/src/features/goals/components/GoalForm.tsx','utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2020,jsx:ts.JsxEmit.ReactJSX}}).outputText;
 vm.runInNewContext(code,{exports,crypto:require('node:crypto').webcrypto,require(name){
  ...
  if(name==='../application/goal-actions')return {goalAction:async()=>({success:'ok'})};
  if(name==='../domain/goal')return {operators:['>=','<=','=','>','<','!=']};
  return web(name);
 }});
 const tree=exports.GoalForm({data:{gameId:'game',definitions:[]},operation:'create'});
...
async function main(){
 ...
 const blocked=render(true).tree;assert.equal(find(blocked,'fieldset')[0].props.disabled,true);assert.equal(find(blocked,'button')[0].props.disabled,true);assert.equal(find(blocked,'button')[0].props.children,'Guardando...');
 blocked.props.onSubmit(event);assert.equal(prevented,2);
 console.log('PASS: immediate duplicate submit blocked; pending controls disabled; Guardando label; stable retry ID; edited input gets a new ID.');
}
