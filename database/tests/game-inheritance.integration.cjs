...
async function main() {
  ...
  await assert.rejects(mutate(second,{operation:'remove_kpi',kpiId:a,confirmed:true}),/heredado/);
  await assert.rejects(mutate(second,{operation:'set_required',kpiId:a,required:false}),/heredado/);
  await assert.rejects(save(second,{[a]:'1'}),/heredado/);
  await assert.rejects(sql`DELETE FROM game_kpis WHERE game_id=${second} AND kpi_definition_id=${a}`,/Inherited/);
  await assert.rejects(sql`UPDATE game_kpis SET required=false WHERE game_id=${second} AND kpi_definition_id=${a}`,/Inherited/);
  await assert.rejects(sql`UPDATE game_state_values SET value=1 WHERE game_id=${second} AND kpi_definition_id=${a}`,/Inherited/);
  ...
  assert.equal((await values(second,'current')).length,4);
  assert.deepEqual(await values(second,'current'),await values(second,'initial'));
  await start.startGame(master,await request(second));
