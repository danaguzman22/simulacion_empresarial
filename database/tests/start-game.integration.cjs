...
async function main(){
 ...
 await lifecycle.resetGame(master,op(next.gameId));assert.equal((await prep.readPreparation(next.gameId,master)).canEdit,false);assert.deepEqual(await valuesOf(next.gameId,'current'),await valuesOf(next.gameId,'initial'));assert.equal((await sql`SELECT count(*)::int n FROM game_kpi_changes WHERE game_id=${next.gameId}`)[0].n,0);
