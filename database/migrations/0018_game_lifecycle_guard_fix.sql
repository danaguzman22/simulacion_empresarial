-- Forward-only corrections to the guards actually bound by 0006-0017.
-- No triggers are disabled. Lifecycle exceptions require a transaction-local,
-- game-scoped context AND the master-authorized lifecycle audit from this transaction.
CREATE FUNCTION public.lifecycle_mutation_allowed(target_game uuid, allowed_operations text[])
RETURNS boolean LANGUAGE sql STABLE SET search_path='' AS $$
 SELECT current_setting('nexus.lifecycle_game_id',true)=target_game::text
 AND current_setting('nexus.lifecycle_operation',true)=ANY(allowed_operations)
 AND EXISTS(SELECT 1 FROM public.game_lifecycle_changes c
 JOIN public.campaign_members m ON m.campaign_id=c.campaign_id AND m.profile_id=c.actor_id AND m.role='master'
 WHERE c.operation=current_setting('nexus.lifecycle_operation',true)
 AND c.transaction_id=pg_current_xact_id()::text
 AND c.details->>'subjectGameId'=target_game::text);
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION public.guard_preparation_change()
RETURNS trigger LANGUAGE plpgsql SET search_path = '' AS $$
DECLARE target public.game_state_sets%ROWTYPE;
BEGIN
  IF TG_OP='DELETE' AND EXISTS(SELECT 1 FROM public.game_state_sets s WHERE s.id=OLD.state_set_id AND public.lifecycle_mutation_allowed(s.game_id,ARRAY['delete'])) THEN RETURN OLD; END IF;
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    IF public.lifecycle_mutation_allowed(OLD.game_id,ARRAY['reset','delete']) THEN
      RETURN OLD;
    END IF;
...
BEGIN
  IF public.lifecycle_mutation_allowed(CASE WHEN TG_OP='DELETE' THEN OLD.game_id ELSE NEW.game_id END,ARRAY['reset','delete']) THEN
    RETURN NULL;
  END IF;
...
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION public.check_current_kpi_state_audited()
RETURNS trigger LANGUAGE plpgsql SET search_path = '' AS $$
BEGIN
 IF public.lifecycle_mutation_allowed(NEW.game_id,ARRAY['reset']) THEN RETURN NULL; END IF;
...
$$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION public.check_game_kpi_audited()
RETURNS trigger LANGUAGE plpgsql SET search_path='' AS $$
...
END; $$;
--> statement-breakpoint
CREATE OR REPLACE FUNCTION public.guard_game_lifecycle_change()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = ''
AS $$
BEGIN
  -- Permit only the FK's ON DELETE SET NULL; no audit content may be edited.
  IF TG_OP='UPDATE' AND OLD.game_id IS NOT NULL AND NEW.game_id IS NULL
   AND pg_trigger_depth()>1 AND (to_jsonb(NEW)-'game_id')=(to_jsonb(OLD)-'game_id')
   AND NOT EXISTS(SELECT 1 FROM public.games WHERE id=OLD.game_id)
   AND public.lifecycle_mutation_allowed(OLD.game_id,ARRAY['delete']) THEN RETURN NEW; END IF;
  IF TG_OP <> 'INSERT' THEN
  ...
  IF NEW.operation IN ('reset','delete') AND NOT EXISTS(
   SELECT 1 FROM public.games g JOIN public.campaign_members m ON m.campaign_id=g.campaign_id
   WHERE g.id=NEW.game_id AND g.campaign_id=NEW.campaign_id AND m.profile_id=NEW.actor_id AND m.role='master'
    AND ((NEW.operation='delete' AND g.status<>'completed') OR (NEW.operation='reset' AND g.status IN ('draft','ready','active','paused','evaluation')))
    AND NEW.details->>'subjectGameId'=g.id::text
    AND NOT EXISTS(SELECT 1 FROM public.games later WHERE later.campaign_id=g.campaign_id AND later.sequence>g.sequence)
  ) THEN RAISE EXCEPTION 'Lifecycle mutation requires the last noncompleted game and its Master'; END IF;
  NEW.transaction_id := pg_current_xact_id()::text;
  NEW.created_at := clock_timestamp();
  RETURN NEW;
END;
...
BEGIN
  IF public.lifecycle_mutation_allowed(OLD.id,ARRAY['reset','delete']) THEN
    RETURN NEW;
  END IF;
...
$$;
--> statement-breakpoint
REVOKE ALL ON FUNCTION public.lifecycle_mutation_allowed(uuid,text[]),public.guard_preparation_change(),public.guard_game_state_value(),public.guard_game_state_set(),public.guard_round_change(),public.guard_game_kpi_change(),public.guard_round_lifecycle(),public.guard_game_period_change(),public.guard_game_kpi_delete_lifecycle(),public.check_current_kpi_value_audited(),public.check_preparation_values_audited(),public.check_current_kpi_state_audited(),public.check_game_kpi_audited(),public.guard_game_lifecycle_change(),public.guard_game_evaluation_transition() FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.lifecycle_mutation_allowed(uuid,text[]) TO service_role;
